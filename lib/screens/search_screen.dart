          ...
          ..._hotels.map((h) => Card(
                child: ListTile(
                  leading: Icon(Icons.hotel,
                      color: Theme.of(context).colorScheme.primary),
                  title: Text(
                    (h['name'] ?? h['title'] ?? 'فندق').toString(),
                    style: const TextStyle(fontWeight: FontWeight.w800),
                  ),
                  subtitle: Text(
                    'المدينة: ${(h['city'] ?? h['location'] ?? 'غير محدد')}',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                  ),
                  onTap: () => _openDetails(
                    title: (h['name'] ?? h['title'] ?? 'فندق').toString(),
                    desc: 'المدينة: ${(h['city'] ?? h['location'] ?? 'غير محدد')}\n'
                        'السعر: ${(h['price_per_night'] ?? h['price'] ?? '—')}',
                    icon: Icons.hotel,
                  ),
                ),
              )),
